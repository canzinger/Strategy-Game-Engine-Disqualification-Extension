...
named ~sge-1.0.3-exe.jar~ and can be executed with

#+BEGIN_SRC sh

java -jar sge-1.0.3-exe.jar
...
follows the POSIX principle (note ~java -jar sge-1.0.3-exe.jar~ is
substituted by ~sge~):
...
The engine has, as of version 1.0.3, two working commands:
...
As of version 1.0.3 ~sge tournament~ supports the following tournament
modes:
...
*** Build environment
 ...
 dependencies {
   compile group: 'at.ac.tuwien.ifs.sge', name: 'sge', version: '1.0.3'
